...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // جلب حالة المستخدم
    // جلب حالة المستخدم، مع التأكد الفعلي من السيرفر
    try {
...
function updateHeaderUI(user) {
    ...
    if (user) {
        ...
        const loggedInDesktopHTML = `
            ...
            <button type="button" onclick="handleLogout(event)" class="cursor-pointer bg-red-600 text-white px-5 py-2.5 lg:mt-0 mt-8 rounded-lg font-bold flex items-center gap-2 hover:bg-red-700 duration-300">
                <span>Log Out</span>
